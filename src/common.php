...
Hook::add('app_init', function () {
    ...
    foreach ($routeArr as $k => $v) {
        ...
        if (is_array($v)) {
            ...
            $_route = $rules[$k] = sprintf($execute, $addon, $controller, $action);
        }
        // 注册路由
        if($_route){
            Route::rule($k,$_route);
...
function get_addon_autoload_config($truncate = false)
{
    ...
    $url_domain_deploy = Config::get('addons.url_domain_deploy');
    ...
    foreach ($addons as $name => $addon) {
        ...
        if ($conf) {
            $rewrite = isset($conf['rewrite']) && is_array($conf['rewrite']) ? $conf['rewrite'] : [];
...
function addon_url($url, $vars = [], $suffix = true, $domain = false)
{
    ...
    $dispatch = (array)think\facade\Request::instance()->dispatch();
